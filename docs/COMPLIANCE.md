...
>
> 本文档依据《参赛手册》2026 年 8 月修订版要求（`GOAI_赛道三手册修订对照表.docx`），
> 完整披露项目的开源计划、商业 API 使用、闭源模型说明、数据来源与授权、第三方依赖等信息。
> **外部资源清单**见独立文档 `docs/EXTERNAL_RESOURCES.md`（13 项，含来源/版本/许可证/替代方案）。
...
- **初赛须一并提交训练与推理源代码**（《参赛手册》2026 年 8 月修订版 §表一·2）。标准：代码完整、流程可复现；随机种子与关键参数在代码中体现；外部资源注明来源与版本。不需要运行环境或容器打包。
- **开源仓库已于 2026-08 上线并公开**：<https://github.com/Alwayshere-su/pi-agent>（公开仓库，MIT 许可），包含：
  - `pi_agent/` + `literature_agent/` + `main.py` 核心代码（23 工具 + 事件驱动状态机）
  - `requirements.txt` 依赖清单（精确版本锁定）
  - 配置文件说明（`.api_key` 模板、环境变量配置，见 `utils/config.py`）
  - 随机种子在代码中体现（`utils/config.py` SEED=42；`seed_everything()` 固定 random/numpy；`literature_agent/discovery.py` 含 `@seed` 内联注释；`symbolic_regression.fit(seed=42)` 默认固定；`scripts/baseline_random_search.py` 显式 `np.random.seed(seed)`）
  - 外部资源来源与版本在代码中体现（`utils/resource_registry.py` 13 项 ExternalResource + `used_in` 字段追踪；7 个模块 docstring 含 `@external` 交叉引用注释；`utils/config.py` 每个 API Key 配置块含 `@external` 注释；`docs/EXTERNAL_RESOURCES.md` 可读版清单）
  - 复现步骤（见 README.md §六、附录 B、附录 D）
...
| 合规项 | 要求 | 状态 | 本文档对应章节 |
|--------|------|------|---------------|
| 初赛提交源代码 | 代码完整、流程可复现；随机种子与关键参数在代码中体现；外部资源注明来源与版本（修订版 §表一·2） | ✅ 已实现 | 第一章 + `utils/resource_registry.py` + `docs/EXTERNAL_RESOURCES.md` |
| 部署/复现说明 | 运行入口、依赖安装、配置方法、随机种子 | ✅ 已提供 | README.md §六 + 附录 B |
| 商业 API 披露 | 调用环节、费用假设、权限范围、可替代性、可复现性影响 | ✅ 已披露 | 第二章 |
| 闭源模型说明 | 使用范围、原因、替代方案、迁移成本、可复现性影响 | ✅ 已说明 | 第三章 |
| 数据来源与授权 | 来源、授权状态、使用方式 | ✅ 已列出 | 第五章 + `docs/EXTERNAL_RESOURCES.md` |
| 第三方依赖披露 | 完整列表、许可证、关键版本 | ✅ 已披露 | 第六章 |
| 数据库不限于推荐列表 | 参赛者可选用任何公开可获取的科学数据库（修订版 §表二·36） | ✅ hMOF 已接入；AFLOW 等列入复赛计划 | 第五章 |
| 密钥管理 | 不入库 | ✅ 已实施 | 第七章 |
| 已有项目声明 | 来源、贡献范围、创新点、协议兼容性 | ✅ 已声明 | 第八章 |
| 证据链可审计 | 结论可追溯至原始文献与 API 调用 | ✅ 已实现 | 第九章 + README.md §五 |
