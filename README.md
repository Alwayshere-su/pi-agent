...
> 已归档运行：`validation`（固态电解质）、`smoke_test` / `g3test`（冒烟验证）、`mof_e2e_v4`（e2e 全量重跑，含 llm_guidance 审计实证）、`mof_rerun` / `mof_rerun_v2`。
...
| [`材料科学文献调研Agent_算法赛初赛方案.docx`](材料科学文献调研Agent_算法赛初赛方案.docx) | 初赛提交方案（最终提交物） |
