...
\textbf{已知工作} & 已有文献确立开放金属位点（OMS）密度与 CO₂ 容量/选择性正相关（p22 钴基 MOF 吡啶位点+OMS、p24 ZIF-8@Zn-MOF-74 核壳），离子液体复合与膜分离可调 CO₂/N₂ 选择性（p65、p67），TSA 循环给出工艺侧权衡（p147）；但双金属组分比例→容量的连续倒 U 定量标度律未见报道 \\
...
\textbf{已知工作} & 已有文献用分子模拟确立水在 OMS 上竞争/水解离占据位点（p186/p182），疏水化功能化可调控胺型 MOF 湿态容量（p224/p210/p201）；但胺型先增后减与 OMS 型单调衰减两类矛盾结论缺乏统一的 RH 定量标度 \\
...
\textbf{已知工作} & 已有文献确立 Qst 与选择性/再生能耗同向权衡的方向（p27 等温吸附热计算方法、p129 DAC 挑战综述），缺陷/核壳工程可调节 Qst（p117、p116）；但容量-选择性-再生能耗三维 Pareto 的 Qst 定量窗口未见系统建立 \\
