...
def _norm(path: str) -> str:
    """规范化路径：绝对化 + 大小写归一（Windows）+ 去除冗余 . / ..。"""
    return os.path.normcase(os.path.normpath(os.path.abspath(path)))


def _is_sensitive_file(path: str) -> bool:
    """敏感文件（密钥/凭据）拦截：.api_key、.env、私钥、证书等。"""
    base = os.path.basename(_norm(path)).lower()
    if base in (".api_key", ".env", "id_rsa", "id_ed25519", "id_ecdsa", "id_dsa"):
        return True
    if base.endswith((".pem", ".key", ".env", ".p12", ".pfx")):
        return True
    return False
...
def read_file(filepath: str, max_chars: int = 80000) -> Dict:
    ...
    # 敏感文件拦截：防止 API Key/私钥被读入对话历史并随 checkpoint/轨迹落盘
    if _is_sensitive_file(filepath):
        return {"exists": False, "path": filepath,
                "error": "Blocked: sensitive file (API key / secret) is not readable.", "content": ""}
...
def write_file(filepath: str, content: str, mode: str = "overwrite") -> Dict:
    ...
    # 敏感文件写保护：防止覆盖/篡改密钥与凭据文件
    if _is_sensitive_file(filepath):
        return {"success": False, "error": f"Blocked: {filepath} is protected (sensitive file)."}

    # Block writes to core agent infrastructure
    # 规范化比较（大小写不敏感）+ 精确边界（agent/ 目录不误伤 agent_backup/）：
    # 原实现用 startswith(abspath("agent")) 会误伤 agent_backup/、且大小写敏感可绕过
    _norm_fp = _norm(filepath)
    for b in (_norm(os.path.abspath("pi_agent")), _norm(os.path.abspath("literature_agent")),
              _norm(os.path.abspath("main.py")), _norm(os.path.abspath("utils")),
              _norm(os.path.abspath("requirements.txt")), _norm(os.path.abspath("pyproject.toml"))):
        if _norm_fp == b or _norm_fp.startswith(b + os.sep):
...
def list_files(directory: str = "workspace", pattern: str = "**/*") -> Dict:
    ...
    # 沙箱：拒绝列出项目根之外的目录（防 ../ 逃逸）
    _root = _norm(_PROJECT_ROOT)
    if _norm(directory) != _root and not _norm(directory).startswith(_root + os.sep):
        return {"directory": directory, "pattern": pattern, "count": 0,
                "total_found": 0, "overflow": False, "files": [],
                "error": f"Blocked: {directory} is outside project root."}
...
def _kill_process_tree(proc) -> None:
    """终止进程树：Windows 用 taskkill /T 杀整树；POSIX 杀进程组（回退单进程 kill）。"""
    try:
        if sys.platform == "win32":
            subprocess.run(
...
def run_shell(command: str, timeout: int = 3600, working_dir: str = None,
    ...
    if _re.search(r'\bfind\s+/(?:\s|$|-)', command.strip(), _re.IGNORECASE):
        return {"success": False, "error": "BLOCKED: find / — use find . or an explicit path", "stdout": "", "stderr": ""}
    ...
    try:
        ...
        _MAX_LINES = 20000    # 缓冲上限：防长时间输出导致内存膨胀
        ...
        def _read_stream(stream, collector):
            for line in iter(stream.readline, ""):
                ...
                if len(collector) > _MAX_LINES:
                    del collector[: len(collector) - _MAX_LINES]
            stream.close()

        # daemon=True：子进程不退出时不会挂住 Python 进程退出
        ...
        try:
            ...
            _kill_process_tree(proc)  # 杀整棵进程树，避免 bash 子进程变孤儿
...
# 全局注册表: pid → {proc, stdout_buf, stderr_buf, start_time, command, status}
_running_processes: dict = {}
_next_pid: int = 0


def _cleanup_all_processes():
    """进程退出时清理全部残留后台子进程（含 Agent 崩溃/异常退出路径）。"""
    for pid in list(_running_processes.keys()):
        try:
            kill_shell(pid)
        except Exception:
            pass
...
def _bg_read_stream(stream, buf: list, lock: _threading.Lock):
    ...
    缓冲有上限（_MAX_BUF_LINES 行）：长时间训练进程 stderr 刷屏时
    只保留最近 N 行，避免内存无限增长。
    """
    _MAX_BUF_LINES = 20000
    try:
        for line in iter(stream.readline, ""):
            with lock:
                buf.append(line)
                if len(buf) > _MAX_BUF_LINES:
                    del buf[: len(buf) - _MAX_BUF_LINES]
    except Exception:
        pass
    finally:
        try:
...
def start_shell(command: str, timeout: int = 3600, working_dir: str = None) -> Dict:
    ...
    try:
        ...
        _running_processes[pid] = {
            ...
            "last_output_time": 0.0,     # 最近一次输出时间（check_shell 僵尸检测用）
        }
